...
function TipoEmpresa() {
  ...
  const columns = useMemo(() => [
    {
      accessorKey: 'nombre_tipoempresa',
      header: 'Nombre',
    },
    {
 ...
 setIdCliente(row.original.id_tipoempresa)
 setNombre(row.original.nombre_tipoempresa);
 ...
 if(row.original.estado ===0){
   ...
   handleEditar(row.original.id_tipoempresa,row.original.nombre_tipoempresa,row.original.descripcion,1);
 }else{
  
   handleEditar(row.original.id_tipoempresa,row.original.nombre_tipoempresa,row.original.descripcion,0);
 }
